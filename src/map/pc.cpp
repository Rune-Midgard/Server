...
bool pc_is_dual_account(struct map_session_data *sd) {
	if (sd == NULL)
		return false;
	if (sd->status.partner_id > 0) {
		int32 uid = gepard_get_unique_id(sd->fd);
		if (uid <= 0)
			return false;
		if (SQL_ERROR == Sql_Query(mmysql_handle, "SELECT ch.name, ch.account_id FROM `char` ch LEFT JOIN login ON ch.account_id = login.account_id WHERE last_unique_id = %d AND ch.online AND ch.account_id != %d; ;", uid ,sd->status.account_id)) {
...
int32 pc_get_unique_id(struct map_session_data *sd) {
	if (sd == NULL)
		return -1;
	if (sd->status.partner_id > 0) {
		return session[sd->fd]->gepard_info.unique_id;
	}
	else
		return 0;
}
