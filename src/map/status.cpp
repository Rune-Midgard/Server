...
int status_damage(struct block_list *src,struct block_list *target,int64 dhp, int64 dsp, int walkdelay, int flag)
{
	...
	if (src && src->type == BL_PC && map_getmapflag(src->m, MF_BATTLEGROUND)) {
		TBL_PC *ssd = BL_CAST(BL_PC,src);
		if (ssd->bg_id) {
			if (target->type == BL_PC) {
				TBL_PC *tssd = BL_CAST(BL_PC,target);
				add2limit(ssd->status.bgstats.damage_done, hp, UINT_MAX);
				add2limit(tssd->status.bgstats.damage_received, hp, UINT_MAX);
				if (ssd->status.bgstats.top_damage < hp)
					ssd->status.bgstats.top_damage = hp;
			} //else if (target->type == BL_MOB) Not Yet
		}
	}
