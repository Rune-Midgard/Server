...
{ // Single Player leaves team
	...
	{ // Update other BG members
		...
		if (battle_config.bg_eAmod_mode)
		{
			...
			if (flag > 0)
				add2limit(pl_sd->status.bgstats.deserter,1,USHRT_MAX);
...
int bg_queue_join(struct map_session_data *sd, int q_id)
{
	...
	if (battle_config.bg_check_dual_account && bg_queue_isdualaccount(sd, qd, sd->bl.id))
	{
		sprintf(output, "One of your account is already in the %s queue.", qd->queue_name);
		clif_displaymessage(sd->fd, output);
		return 0;
	}
...
bool bg_queue_isdualaccount(struct map_session_data *sd, struct queue_data *qd, int id)
{
	struct queue_member *head;
	nullpo_retr(0, qd);
	int32 current_uid = gepard_get_unique_id(sd->fd);
	if (current_uid <= 0)
		return false; //gepard desactivated, we don't check

	head = qd->first;
	while (head != NULL)
	{
		if (head->sd && gepard_get_unique_id(head->sd->fd) == current_uid) {
			return true;
		}
		head = head->next;
	}
